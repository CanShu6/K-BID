...
from parsers.parser_sur import parse_sur_args
...
def train(args):
    ...
    model = SUR(args, data.n_users, data.n_entities, data.n_relations, data.A_in, user_pre_embed, item_pre_embed)
...
def predict(args):
    ...
    # load model
    model = SUR(args, data.n_users, data.n_entities, data.n_relations)
    model = load_model(model, args.pretrain_model_path)
    model.to(device)
...
def save(args):
    ...
    def output(all_embed):
        ...
        user_embed = all_embed[:data.n_users, :]
        item_embed = all_embed[data.n_users:data.n_items, :]
    ...
    # export embeddings
    all_embeddings = model.calc_social_embeddings()
    output(all_embeddings)

    all_embeddings = model.entity_embed.weight
    output(all_embeddings)
