...
class KGAT(nn.Module):
    ...
    def __init__(self, args, n_users, n_entities, n_relations, A_in=None,
        ...
        if (self.use_pretrain == 1) and (user_pre_embed is not None) and (item_pre_embed is not None):
            other_entity_embed = nn.Parameter(torch.Tensor(self.n_entities - item_pre_embed.shape[0] - user_pre_embed.shape[0], self.embed_dim))
    ...
    def calc_social_loss(self, inviter_ids, voter_pos_ids, voter_neg_ids):
        ...
        # Equation (13)
        # social_loss = F.softplus(neg_score - pos_score)
