...
def evaluate(model, dataloader, Ks, device):
    ...
    with tqdm(total=len(inviter_batches), desc='Evaluating Iteration') as pbar:
        for batch_id, batch_inviters in enumerate(inviter_batches):
            ...
            with torch.no_grad():
                batch_scores = model(batch_inviters, user_ids, is_train=False)       # (n_batch_users, n_items)

            batch_metrics = calc_metrics_at_k(batch_scores.cpu(), label_voter_batches[batch_id],
...
def train(args):
    ...
    model = CKE(args, data.n_users, data.n_entities, data.n_relations, user_pre_embed)
    ...
    for epoch in range(args.n_epoch):
        ...
        time1 = time()
        total_loss = 0
        n_batch = data.n_social_train // data.social_batch_size + 1
        ...
        for iter in tqdm(range(1, n_batch + 1), desc='Training'):
            ...
            batch_loss = model(batch_inviters, batch_pos_voters, batch_neg_voters, kg_batch_head, kg_batch_relation, kg_batch_pos_tail, kg_batch_neg_tail, is_train=True)
